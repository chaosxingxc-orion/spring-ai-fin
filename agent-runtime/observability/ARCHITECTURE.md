...
| **AD-8: Audit broken out into `agent-runtime/audit/`** | `observability/` no longer owns AuditLogger, WormAnchor, hash chain, or PII reveal protocol | The security boundary between best-effort telemetry (never raises) and mandatory regulatory evidence (must raise) must be sharp. P0-8 closure. |
