...
This is the M4 fix from the v5.0 review — v5.0 proposed a three-layer FIBO ontology (Apache Jena + Protégé + SPARQL + JSON-LD + SHACL + ontology2nebula); v6.0 ships **PostgreSQL JSONB + a 30–50 class hand-curated finance glossary** + four-layer retrieval (grep → BM25 → JSONB filter → optional vector).
...
- The proposed Apache Jena + Protégé + SPARQL + JSON-LD + SHACL + custom `ontology2nebula` converter + FIBO MCP server stack is multi-year work added on day 0
