...
|---|---|---|
| **Senior architect** | sec-1 (purpose) -> sec-3 (three-tier layering) -> sec-6 (decision chains) -> sec-11 (quality bar) | 90 min |
| **Security reviewer** | sec-1 (boundary) -> sec-4.4 (auth/tenancy) -> sec-6 (security decisions D-7, D-12, D-15) -> sec-10 (security cross-cutting) -> sec-12 (risks) | 60 min |
| **Compliance reviewer** | sec-1 (positioning) -> sec-2 (regulatory landscape) -> sec-5 (data plane) -> sec-7 (audit & immutability) -> sec-11.4 (compliance bar) | 75 min |
| **Operations / SRE** | sec-3 (deployment shapes) -> sec-4 (building blocks) -> sec-8 (runtime scenarios) -> sec-10.5 (operator-shape gate) -> sec-13 (capacity & DR) | 90 min |
| **Skeptical reviewer** | sec-6 (every decision lists alternatives + why this won) -> sec-12 (risks & technical debt) -> `docs/architecture-v5.0-review-2026-05-07.md` (the predecessor critique that produced this revision) | 60 min |
...
| FIBO three-layer ontology (Apache Jena + Protege + SPARQL + JSON-LD + SHACL + ontology2nebula) | **Deferred.** v1 ships PostgreSQL JSONB + 30-50 class hand-curated glossary as static prompt context. Phase-2 trigger: a named regulator demands semantic-graph-based reasoning evidence. | M4 |
...
| 5 interaction modes for human-AI collaboration | **Reframed as 2 primitives x 4 side-effect levels.** Information flow + authority transfer; surface variants per side-effect class (L1-L4). | M3 |
...
| Outbox-as-universal-cross-store-transaction-model | **Split into Outbox-async (telemetry; eventual within tenant) / Sync-Saga (cross-entity progress with explicit compensations and reconciliation; not ACID across entities) / Direct-DB (read-your-write; LEDGER_ATOMIC double-entry within one transaction).** Every write site declares `consistency` annotation; financial writes also declare `FinancialWriteClass` (see `agent-runtime/outbox/ARCHITECTURE.md` sec-6). | H6 |
...
- **Kafka / Istio / multi-cluster mesh** -- Tier-2 components, deferred until service count or async fan-out justifies (named trigger criteria in sec-12).
...
### How to evaluate sec-2

- **A2.1**: "Compliance-by-design with named regulator owners" -- is the mapping complete enough that an OJK or MAS auditor walking in can be answered? (We argue: yes, with sec-11.4 providing the audit-trail evidence and sec-15 of the predecessor review covering the open gaps.)
...
- **A2.4**: "Kafka deferred until needed" -- is this realistic for SEA financial-services scale? (We argue: Tier-1 customer = single bank with O(100) agents = O(10K) runs/day = single-process Postgres outbox is sufficient. Trigger criterion for Kafka adoption is in sec-12.)
...
| `starters/` | Spring Boot Starters published as customer-facing SDK modules | AS-DX | (see sec-7 SDK contract) |
...
- **Detail**: [`agent-runtime/outbox/ARCHITECTURE.md`](agent-runtime/outbox/ARCHITECTURE.md) sec-Financial Classes
...
| **PII redaction** | Microsoft Presidio integration in `agent-runtime/audit/PiiRedactor.java`. PII tokens replaced with format-preserving tokens (FFP3) at write; decoded only via dual-approval workflow (sec-5.5). | RO + GOV | `agent-runtime/audit/PiiRedactor.java`; `agent-runtime/audit/TokenizationService.java` |
...
- **A7.4**: "Spine validator runs at constructor" -- does this slow construction in a hot loop? (Profiled in hi-agent: <1us per validation; negligible.)
...
- Accepting agent run requests via frozen `/v1/*` HTTP contract (sec-5.1).
- Executing 5-stage TRACE runs with cancellation, restart-survival, and SSE streaming (sec-5.1, sec-5.2).
- Dispatching to multiple agent frameworks via `FrameworkAdapter` (sec-5.3).
- Persisting events via outbox + sync-saga with explicit consistency annotations (sec-5.4).
- Honouring HITL gates and dual-approval workflows (sec-5.5).

#### 8.3.2 Security review

- **AuthN**: Posture-aware JWT validation per D-block sec-A3. Research SaaS multi-tenant + prod require RS256 / ES256 with JWKS; HS256 is permitted only on DEV loopback or as an explicit BYOC single-tenant carve-out documented in `docs/governance/allowlists.yaml`. `alg=none` and HS/RS confusion are always rejected. JWT claims include `iss`, `sub` (userId), `tenantId`, `projectId`, `roles`, `aud`, `exp`, `nbf`, `iat`, `jti`, `kid` (asymmetric only). Anti-forgery cross-check between body and JWT.
...
- **Capacity planning**: sec-13 reference sizing tiers (XS / S / M / L / XL).
...
- **Inference cost**: Tier router routes by purpose x complexity x budget x confidence; multi-provider failover; prompt-cache injection.
...
Four shadow-path tests per layer -- `(happy / nil / empty / error)` x layer in {filter, controller, facade, kernel, store, gRPC adapter, MCP transport, Kafka producer/consumer (future), audit emitter, metrics tag}.
...
| 3 | N>=3 sequential real-LLM runs | `gate/run_seq_runs.sh` | Each reaches `state=done` in <= `2 x observed_p95`; `llm_fallback_count == 0`; >=1 LLM request in access log + metric. |
...
- **Pin Behavior Version**: v5.0 marked Developer + Operator + Compliance as [x]; v6.0 restricts to Compliance + signed pinning addendum (D-11).
...
| **TenantContext propagation across all layers not validated** | 4 shadow-path tests x 10 layers = 40 tests don't exist. | AS-RO + RO | W2 deliverable (after RLS connection protocol from `agent-runtime/server/` sec-6 lands). |
...
| Apache Jena + FIBO + Protege full stack | Deferred to phase 2; v1 = JSONB glossary | M4 |
...
### How to evaluate sec-12

- **A12.1**: "All v1-blocking risks have named owners and W0/W1/W2/W3 plans" -- what's missing? (Reviewers should check whether any sec-1-sec-10 claim is unbacked by sec-12 risk tracking; the wave plan is `docs/plans/roadmap-W0-W4.md`.)
...
| TenantContext | Authenticated identity context; carries `tenantId`, `userId`, `projectId`; binding contract specified in sec-10.2 |
...
| SAS-1 / SAS-2 / SAS-3 / SAS-4..10 | spring-ai-fin layering rules (see sec-3.2) -- mirror hi-agent R-AS-1/2/3/4..10 |
