...
| C3 | Tenant binding | `agent-platform/tenant` | `TenantBinder` filter + `TransactionSynchronization` | required in `research`/`prod` | `TenantIsolationIT` | W1 | RLS hides rows |
| C4 | RLS row visibility | Postgres + `agent-platform/tenant` | RLS policy on every tenant table | `research`/`prod` enforce | `RlsPolicyCoverageIT` | W1 | empty result |
| C5 | RLS GUC required | Postgres trigger | `BEFORE INSERT/UPDATE` on tenant tables | `research`/`prod` enforce | `GucEmptyAtTxStartIT` | W1 | tx fails |
