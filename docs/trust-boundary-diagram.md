...
> [docs/cross-cutting/trust-boundary-diagram.md](cross-cutting/trust-boundary-diagram.md). 
...
| **Failure behaviour** | FailoverChain to next provider; emit `springAiAscend_llm_fallback_total` |
...
- Every boundary referenced in `docs/cross-cutting/security-control-matrix.md` is in this diagram
