...
- `AppPosture` — read by `AppPosture.fromEnv()` once in bootstrap and injected into `agent-platform/facade/PostureAwareValidator`. **Contracts are environment-free** — the `contracts/` module never imports `agent-runtime.posture` AND never calls `System.getenv` / `Environment.getProperty` / `AppPosture.fromEnv` (per `agent-platform/contracts/ARCHITECTURE.md` §AD-2 and §AD-7; cycle-2 correction). Posture-conditional behaviour lives in the facade validator, not in record canonical constructors.
...
|---|---|
| **Posture (Rule 11)** | `APP_POSTURE={dev,research,prod}` (default `dev`). Read once in bootstrap via `AppPosture.fromEnv` and injected into facade validators. **Contracts records are environment-free** (per `agent-platform/contracts/ARCHITECTURE.md` §AD-2 and §AD-7); posture-conditional behaviour lives in `agent-platform/facade/PostureAwareValidator` which receives an injected `AppPosture`. Spring profile activation via `@Profile` tied to posture. |
...
| **Error envelope** | `agent-platform/contracts/v1/errors/ContractError.java` (record/DTO; never thrown) and `ContractException.java` (RuntimeException carrying ContractError; thrown). Categories: `validation`, `auth`, `tenantScope`, `idempotencyConflict`, `notFound`, `rateLimit`, `internal`, `spineCompleteness`. Spring `@ControllerAdvice` maps all `ContractException` subclasses to `ContractError` envelope. See `agent-platform/contracts/ARCHITECTURE.md` §AD-5. |
